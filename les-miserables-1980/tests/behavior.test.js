...
const cursorJs = fs.readFileSync(path.join(root, "..", "shared", "cursors", "les-miserables-1980.js"), "utf8");
const cursorMarker = undefined;
const cursorProfile = {"motif":"revolutionFlag","trail":"magicDust","burst":"crispShockwave","motion":"still","accent":"#e6c472","size":54,"follow":0.52,"emitDistance":16,"clickOn":"down","burstParticles":4,"hotspot":[0.24,0.54],"primary":"#b33a35","secondary":"#e6c472"};
...
test("page includes a themed canvas cursor effect", () => {
  ...
  assert.match(indexHtml, /\.\.\/shared\/cursors\/les-miserables-1980\.js/);
...
test("The Greatest Show compresses the opening vocalization and shows its repeat count", () => {
  if ("les-miserables-1980" !== "the-greatest-showman") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Epic keeps all 40 songs and removes non-lyric stage directions", () => {
  if ("les-miserables-1980" !== "epic-the-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Dear Evan Hansen keeps slash-delimited line IPA and its upper-left note hotspot", () => {
  if ("les-miserables-1980" !== "dear-evan-hansen") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("bracketed lyrics with aligned translations remain lyrics, not speaker labels", () => {
  if ("les-miserables-1980" !== "mozart-opera-rock") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
...
test("Starmania opening keeps the reviewed Chinese translation", () => {
  if ("les-miserables-1980" !== "starmania") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Phantom and Love Never Dies stay in separate source ranges", () => {
  if (!["phantom-of-the-opera", "love-never-dies"].includes("les-miserables-1980")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const songs = sandbox.window.songs;
  if ("les-miserables-1980" === "phantom-of-the-opera") {
...
test("Romeo Aimer keeps one opening lyric, not an expanded spelling duplicate", () => {
  if ("les-miserables-1980" !== "romeo-et-juliette") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Notre-Dame speaker metadata never enters the lyric IPA or speech input", () => {
  if ("les-miserables-1980" !== "notre-dame-de-paris") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("reviewed OCR word fragments are reassembled", () => {
  if (!["la-legende-du-roi-arthur", "phantom-of-the-opera", "notre-dame-de-paris", "le-roi-soleil", "mozart-opera-rock"].includes("les-miserables-1980")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
  ...
  if ("les-miserables-1980" === "la-legende-du-roi-arthur") {
  ...
  if ("les-miserables-1980" === "notre-dame-de-paris") {
...
test("Le Roi Soleil keeps clean lyric breaks and real glosses", () => {
  if ("les-miserables-1980" !== "le-roi-soleil") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
...
test("Moliere keeps the requested show name and dictionary meaning", () => {
  if ("les-miserables-1980" !== "moliere-le-spectacle-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(wordDataJs, sandbox);
