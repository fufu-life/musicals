...
const cursorJs = fs.readFileSync(path.join(root, "..", "shared", "cursors", "phantom-of-the-opera.js"), "utf8");
const cursorMarker = "preRenderGrandChandelier";
const cursorProfile = {"motif":"grandChandelier","trail":"crystalGlint","burst":"pressGlow","motion":"still","accent":"#7d1119","size":50,"follow":0.35,"emitDistance":10,"clickOn":"down","burstParticles":6,"hotspot":[0.5,0.5],"primary":"#c7ab67","secondary":"#f1eee7"};
...
test("page includes a themed canvas cursor effect", () => {
  ...
  assert.match(indexHtml, /\.\.\/shared\/cursors\/phantom-of-the-opera\.js/);
...
test("The Greatest Show compresses the opening vocalization and shows its repeat count", () => {
  if ("phantom-of-the-opera" !== "the-greatest-showman") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Epic keeps all 40 songs and removes non-lyric stage directions", () => {
  if ("phantom-of-the-opera" !== "epic-the-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Dear Evan Hansen keeps slash-delimited line IPA and its upper-left note hotspot", () => {
  if ("phantom-of-the-opera" !== "dear-evan-hansen") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("bracketed lyrics with aligned translations remain lyrics, not speaker labels", () => {
  if ("phantom-of-the-opera" !== "mozart-opera-rock") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
...
test("Starmania opening keeps the reviewed Chinese translation", () => {
  if ("phantom-of-the-opera" !== "starmania") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Phantom and Love Never Dies stay in separate source ranges", () => {
  if (!["phantom-of-the-opera", "love-never-dies"].includes("phantom-of-the-opera")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const songs = sandbox.window.songs;
  if ("phantom-of-the-opera" === "phantom-of-the-opera") {
...
test("Romeo Aimer keeps one opening lyric, not an expanded spelling duplicate", () => {
  if ("phantom-of-the-opera" !== "romeo-et-juliette") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Notre-Dame speaker metadata never enters the lyric IPA or speech input", () => {
  if ("phantom-of-the-opera" !== "notre-dame-de-paris") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("reviewed OCR word fragments are reassembled", () => {
  if (!["la-legende-du-roi-arthur", "phantom-of-the-opera", "notre-dame-de-paris", "le-roi-soleil", "mozart-opera-rock"].includes("phantom-of-the-opera")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
  ...
  if ("phantom-of-the-opera" === "la-legende-du-roi-arthur") {
  ...
  if ("phantom-of-the-opera" === "notre-dame-de-paris") {
...
test("Le Roi Soleil keeps clean lyric breaks and real glosses", () => {
  if ("phantom-of-the-opera" !== "le-roi-soleil") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
...
test("Moliere keeps the requested show name and dictionary meaning", () => {
  if ("phantom-of-the-opera" !== "moliere-le-spectacle-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(wordDataJs, sandbox);
