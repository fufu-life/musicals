...
const cursorJs = fs.readFileSync(path.join(root, "..", "shared", "cursors", "mozart-opera-rock.js"), "utf8");
const cursorMarker = "preRenderNeonNote";
const cursorProfile = {"primary":"#f03c96","secondary":"#60e6d1"};
...
test("page includes a themed canvas cursor effect", () => {
  ...
  assert.match(indexHtml, /\.\.\/shared\/cursors\/mozart-opera-rock\.js/);
...
test("The Greatest Show compresses the opening vocalization and shows its repeat count", () => {
  if ("mozart-opera-rock" !== "the-greatest-showman") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Epic keeps all 40 songs and removes non-lyric stage directions", () => {
  if ("mozart-opera-rock" !== "epic-the-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Dear Evan Hansen keeps slash-delimited line IPA and its upper-left note hotspot", () => {
  if ("mozart-opera-rock" !== "dear-evan-hansen") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("bracketed lyrics with aligned translations remain lyrics, not speaker labels", () => {
  if ("mozart-opera-rock" !== "mozart-opera-rock") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
...
test("Starmania opening keeps the reviewed Chinese translation", () => {
  if ("mozart-opera-rock" !== "starmania") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Phantom and Love Never Dies stay in separate source ranges", () => {
  if (!["phantom-of-the-opera", "love-never-dies"].includes("mozart-opera-rock")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const songs = sandbox.window.songs;
  if ("mozart-opera-rock" === "phantom-of-the-opera") {
...
test("Romeo Aimer keeps one opening lyric, not an expanded spelling duplicate", () => {
  if ("mozart-opera-rock" !== "romeo-et-juliette") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Notre-Dame speaker metadata never enters the lyric IPA or speech input", () => {
  if ("mozart-opera-rock" !== "notre-dame-de-paris") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("reviewed OCR word fragments are reassembled", () => {
  if (!["la-legende-du-roi-arthur", "phantom-of-the-opera", "notre-dame-de-paris", "le-roi-soleil", "mozart-opera-rock"].includes("mozart-opera-rock")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
  ...
  if ("mozart-opera-rock" === "la-legende-du-roi-arthur") {
  ...
  if ("mozart-opera-rock" === "notre-dame-de-paris") {
...
test("Le Roi Soleil keeps clean lyric breaks and real glosses", () => {
  if ("mozart-opera-rock" !== "le-roi-soleil") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
...
test("Moliere keeps the requested show name and dictionary meaning", () => {
  if ("mozart-opera-rock" !== "moliere-le-spectacle-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(wordDataJs, sandbox);
