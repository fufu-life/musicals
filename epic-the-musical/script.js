const SETTINGS_KEY = "epic-the-musical-display-settings";
const CURRENT_SONG_KEY = "epic-the-musical-current-song";
const SIDEBAR_KEY = "epic-the-musical-sidebar-collapsed";
const PLAYBACK_RATE_KEY = "epic-the-musical-playback-rate";
...
function initThemedCursor() {
  ...
  function addBurst(x, y) {
    ...
    const ringKinds = new Set(["roseWindowGlow", "loveRipples", "sunHalo", "moonHalo", "softGreenRipple", "softOceanWave"]);
    if (ringKinds.has(click)) {
      const count = click === "roseWindowGlow" ? 2 : click === "softGreenRipple" || click === "softOceanWave" ? 1 : 3;
...
function drawParticles(ctx, particles, bursts, colors) {
  ...
  for (let index = bursts.length - 1; index >= 0; index -= 1) {
    ...
    b.life -= b.kind === "letterfall" ? 0.025 : /Glow|Ripples|Halo|softGreenRipple|softOceanWave/.test(b.kind) ? 0.045 : 0.065;
    b.x += b.vx;
    b.y += b.vy;
    b.vy += b.kind === "letterfall" ? 0.018 : b.kind === "tricolorConfetti" || b.kind === "inkDrops" ? 0.05 : 0.006;
    b.radius += /Glow|Ripples|Halo|softGreenRipple|softOceanWave/.test(b.kind) ? 0.75 : 0.35;
    if (b.life <= 0) {
    ...
    } else if (["roseWindowGlow", "loveRipples", "sunHalo", "moonHalo", "softGreenRipple", "softOceanWave"].includes(b.kind)) {
      const softRipple = b.kind === "softGreenRipple" || b.kind === "softOceanWave";
      const ringColor = b.kind === "softGreenRipple" ? colors.primary : b.kind === "softOceanWave" ? "#2d9fb6" : b.color;
      ctx.globalCompositeOperation = "screen";
