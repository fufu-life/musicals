(() => {
  ...
  const config = {"motif":"stoneTablets","trail":"goldDust","burst":"sunHalo","motion":"still","accent":"#c89b45","size":56,"follow":0.5,"emitDistance":18,"clickOn":"down","burstParticles":4,"hotspot":[0.5,0.5],"primary":"#1d8aa6","secondary":"#f0e6c5"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderStoneTablets() {
  ...
  preRenderStoneTablets();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderStoneTablets()).observe(document.documentElement, {
      attributes: true,
