(() => {
  ...
  const config = {"motif":"voteButton","trail":"none","burst":"subtleRing","motion":"still","accent":"#e3ba25","size":58,"follow":0.46,"emitDistance":16,"clickOn":"up","burstParticles":0,"hotspot":[0.5,0.5],"primary":"#f0c62b","secondary":"#7e4fa1"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  if (config.motif === "comeFromAwayGlobe" || config.motif === "legallyBlondeBalance") {
    new MutationObserver(() => preRenderVoteButton()).observe(document.documentElement, {
      attributes: true,
