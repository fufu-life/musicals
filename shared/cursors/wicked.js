(() => {
  ...
  const config = {"motif":"wickedHat","trail":"none","burst":"softGreenRipple","motion":"still","accent":"#8dc63f","size":56,"follow":0.58,"emitDistance":1000000000,"clickOn":"up","burstParticles":0,"hotspot":[0.5,0.5],"primary":"#8dc63f","secondary":"#f4f0df"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  if (config.motif === "comeFromAwayGlobe" || config.motif === "legallyBlondeBalance") {
    new MutationObserver(() => preRenderWickedHat()).observe(document.documentElement, {
      attributes: true,
