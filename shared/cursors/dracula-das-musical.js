(() => {
  ...
  const config = {"motif":"draculaBat","trail":"batEmbers","burst":"batFlare","motion":"still","accent":"#a82135","size":58,"follow":0.42,"emitDistance":15,"clickOn":"down","burstParticles":6,"hotspot":[0.5,0.5],"primary":"#8f1d2c","secondary":"#d7b36a"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  preRenderDraculaBat();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderDraculaBat()).observe(document.documentElement, {
      attributes: true,
