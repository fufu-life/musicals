(() => {
  ...
  const config = {"motif":"filmReel","trail":"none","burst":"none","motion":"turn","accent":"#d78024","size":54,"follow":0.58,"emitDistance":1000000000,"clickOn":"none","burstParticles":0,"hotspot":[0.5,0.5],"primary":"#d78024","secondary":"#f1d7a2"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderFilmReel() {
    ...
    if (config.motif === "comeFromAwayGlobe") {
      ...
      cacheCtx.stroke();
      cacheCtx.shadowBlur = 0;
      cacheCtx.fillStyle = "#120509";
      cacheCtx.beginPath();
      cacheCtx.ellipse(0, 2, 4.6, 16, 0, 0, Math.PI * 2);
      cacheCtx.fill();
      cacheCtx.beginPath();
      cacheCtx.arc(0, -9, 6.5, 0, Math.PI * 2);
      cacheCtx.fill();
      cacheCtx.beginPath();
      cacheCtx.moveTo(-5, -12);
  ...
  preRenderFilmReel();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderFilmReel()).observe(document.documentElement, {
      attributes: true,
