(() => {
  ...
  const config = {"motif":"inspirationPoint","trail":"fiveLineStaff","burst":"goldenRippleNotes","motion":"still","accent":"#ffd700","size":58,"follow":0.6,"emitDistance":8,"clickOn":"down","burstParticles":0,"hotspot":[0.5,0.5],"primary":"#d6a52f","secondary":"#f2ead5"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderInspirationPoint() {
  ...
  preRenderInspirationPoint();
  if (config.motif === "comeFromAwayGlobe" || config.motif === "legallyBlondeBalance") {
    new MutationObserver(() => preRenderInspirationPoint()).observe(document.documentElement, {
      attributes: true,
