(() => {
  ...
  const config = {"motif":"matildaPencil","trail":"none","burst":"subtleRing","motion":"still","accent":"#df72a6","size":56,"follow":0.54,"emitDistance":1000000000,"clickOn":"down","burstParticles":0,"hotspot":[0.29,0.72],"primary":"#df72a6","secondary":"#e8c56a"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderMatildaPencil() {
    ...
    if (config.motif === "comeFromAwayGlobe") {
      ...
      cacheCtx.save();
      const lightTheme = document.documentElement.dataset.musicalTheme === "light";
      const noteGlow = lightTheme ? "rgba(184,134,11,0.22)" : "rgba(229,201,120,0.28)";
      cacheCtx.fillStyle = noteGlow;
      cacheCtx.shadowColor = lightTheme ? "rgba(184,134,11,0.35)" : "rgba(229,201,120,0.5)";
      cacheCtx.shadowBlur = 12;
      cacheCtx.beginPath();
      cacheCtx.arc(0, 0, 18, 0, Math.PI * 2);
      cacheCtx.fill();
      cacheCtx.shadowBlur = 0;
      cacheCtx.font = '42px "Apple Color Emoji", "Segoe UI Emoji", sans-serif';
      cacheCtx.textAlign = "center";
      cacheCtx.textBaseline = "middle";
      cacheCtx.shadowColor = lightTheme ? "rgba(96,71,10,0.42)" : "rgba(229,201,120,0.72)";
      cacheCtx.shadowBlur = 6;
      cacheCtx.fillText("🎵", 0, 0);
      cacheCtx.shadowBlur = 0;
      cacheCtx.restore();
  ...
  preRenderMatildaPencil();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderMatildaPencil()).observe(document.documentElement, {
      attributes: true,
