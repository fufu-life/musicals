(() => {
  ...
  const config = {"motif":"rentGraffiti","trail":"neonSpark","burst":"subtleRipple","motion":"still","accent":"#c51d49","size":58,"follow":0.55,"emitInterval":35,"clickOn":"down","burstParticles":2,"hotspot":[0.1875,0.1875],"primary":"#c51d49","secondary":"#f0d65d"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderRentGraffiti() {
  ...
  preRenderRentGraffiti();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderRentGraffiti()).observe(document.documentElement, {
      attributes: true,
