(() => {
  ...
  const config = {"motif":"castNote","trail":"diamondDust","burst":"softDiamondGlow","motion":"still","accent":"#2a7aaa","size":56,"follow":0.52,"emitDistance":18,"clickOn":"down","burstParticles":3,"hotspot":[0.27,0.27],"rotation":-0.7853981633974483,"primary":"#2676a8","secondary":"#edf5f7"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  preRenderCastNote();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderCastNote()).observe(document.documentElement, {
      attributes: true,
