(() => {
  ...
  const config = {"motif":"tickClock","trail":"clockTicks","burst":"clockShockwave","motion":"turn","accent":"#f3b33d","size":54,"follow":0.6,"emitDistance":8,"clickOn":"down","burstParticles":0,"hotspot":[0.5,0.5],"primary":"#f3b33d","secondary":"#f5e4a8"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  preRenderTickClock();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderTickClock()).observe(document.documentElement, {
      attributes: true,
