(() => {
  ...
  const config = {"motif":"windingKey","trail":"neonSpark","burst":"subtleRipple","motion":"still","accent":"#7454ae","size":58,"follow":0.55,"emitInterval":35,"clickOn":"down","burstParticles":2,"hotspot":[0.1875,0.1875],"primary":"#4c9cc0","secondary":"#e8f4f7"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  if (config.motif === "comeFromAwayGlobe" || config.motif === "legallyBlondeBalance") {
    new MutationObserver(() => preRenderWindingKey()).observe(document.documentElement, {
      attributes: true,
