(() => {
  ...
  const config = {"motif":"blackStar","trail":"glitchPixel","burst":"glitchRipple","motion":"still","accent":"#98d9ea","size":82,"follow":0.45,"emitInterval":30,"clickOn":"up","burstParticles":6,"hotspot":[0.5,0.5],"primary":"#3fb6d8","secondary":"#f0d65d"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  preRenderBlackStar();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderBlackStar()).observe(document.documentElement, {
      attributes: true,
