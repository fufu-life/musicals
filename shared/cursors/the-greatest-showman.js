(() => {
  ...
  const config = {"motif":"marqueeHat","trail":"marqueeGold","burst":"curtainCall","motion":"still","accent":"#d59b3a","size":60,"follow":0.46,"emitDistance":22,"clickOn":"up","burstParticles":7,"hotspot":[0.5,0.5],"primary":"#d59b3a","secondary":"#e75b3c"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  preRenderMarqueeHat();
  if (config.motif === "comeFromAwayGlobe") {
    new MutationObserver(() => preRenderMarqueeHat()).observe(document.documentElement, {
      attributes: true,
