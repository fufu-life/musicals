(() => {
  ...
  const config = {"motif":"ludwigCastle","trail":"ludwigStarlight","burst":"castleGlow","motion":"still","accent":"#8aa4c8","size":60,"follow":0.45,"emitDistance":22,"clickOn":"up","burstParticles":5,"hotspot":[0.5,0.5],"primary":"#8aa4c8","secondary":"#e6d6a0"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderLudwigCastle() {
  ...
  preRenderLudwigCastle();
  if (config.motif === "comeFromAwayGlobe" || config.motif === "legallyBlondeBalance") {
    new MutationObserver(() => preRenderLudwigCastle()).observe(document.documentElement, {
      attributes: true,
