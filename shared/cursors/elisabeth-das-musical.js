(() => {
  ...
  const config = {"motif":"classicTiara","trail":"diamondDust","burst":"softDiamondGlow","motion":"still","accent":"#17121f","size":58,"follow":0.45,"emitDistance":25,"clickOn":"up","burstParticles":3,"hotspot":[0.5,0.5],"primary":"#b5b2bf","secondary":"#d8b56d"};
  const particles = [];
  const bursts = [];
  const trailPoints = [];
  ...
  function preRenderClassicTiara() {
  ...
  preRenderClassicTiara();
  if (config.motif === "comeFromAwayGlobe" || config.motif === "legallyBlondeBalance") {
    new MutationObserver(() => preRenderClassicTiara()).observe(document.documentElement, {
      attributes: true,
