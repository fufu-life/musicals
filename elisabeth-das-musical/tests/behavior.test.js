...
const cursorJs = fs.readFileSync(path.join(root, "..", "shared", "cursors", "elisabeth-das-musical.js"), "utf8");
const cursorMarker = "preRenderClassicTiara";
const cursorProfile = {"motif":"classicTiara","trail":"diamondDust","burst":"softDiamondGlow","motion":"still","accent":"#17121f","size":58,"follow":0.45,"emitDistance":25,"clickOn":"up","burstParticles":3,"hotspot":[0.5,0.5],"primary":"#b5b2bf","secondary":"#d8b56d"};
...
test("page includes a themed canvas cursor effect", () => {
  ...
  assert.match(indexHtml, /\.\.\/shared\/cursors\/elisabeth-das-musical\.js/);
  ...
  if ("elisabeth-das-musical" === "notre-dame-de-paris") {
...
test("Matilda My House keeps complete phrases together and preserves the backing vocal", () => {
  if ("elisabeth-das-musical" !== "matilda") return;
...
test("Sound of Music uses a polished blue vector music-note cursor", () => {
  if ("elisabeth-das-musical" !== "sound-of-music-the") return;
...
test("Preludium keeps the reviewed Latin-to-Chinese line pairs", () => {
  if ("elisabeth-das-musical" !== "sound-of-music-the") return;
  const sourceText = fs.readFileSync(path.join(root, "..", "..", "lyrics", "Elisabeth - Das Musical - Gesamtaufnahme Live - Jubiläumsfassung (37313354).md"), "utf8");
...
test("Come From Away keeps the reviewed count-in and globe cursor", () => {
  if ("elisabeth-das-musical" !== "come-from-away") return;
...
test("Les Misérables keeps the English additions in concert order", () => {
  if ("elisabeth-das-musical" !== "les-miserables") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("The Greatest Show compresses the opening vocalization and shows its repeat count", () => {
  if ("elisabeth-das-musical" !== "the-greatest-showman") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Epic keeps all 40 songs and removes non-lyric stage directions", () => {
  if ("elisabeth-das-musical" !== "epic-the-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const songs = sandbox.window.songs;
...
test("Dear Evan Hansen keeps slash-delimited line IPA and its upper-left note hotspot", () => {
  if ("elisabeth-das-musical" !== "dear-evan-hansen") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("bracketed lyrics with aligned translations remain lyrics, not speaker labels", () => {
  if ("elisabeth-das-musical" !== "mozart-opera-rock") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
...
test("Elisabeth Chinese translations keep speaker labels out of lyric text", () => {
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const speakerLabels = [
    "一粒沙", "死者", "索菲", "弗兰茨", "马科斯", "鲁道夫", "索菲&路德维卡", "马克斯", "弗朗茨·约瑟夫", "其他死者", "青年鲁道夫", "幼年鲁道夫",
    "弗朗茨·约瑟夫&鲁道夫&马克斯&索菲&路德维卡", "所有死者", "死神", "皇太后", "皇帝", "皇太后索菲", "路德维卡", "海伦娜", "路德维卡公爵夫人",
    "伊丽莎白叔父", "伊丽莎白的叔父", "叔父和舅父们", "一对夫妇", "其他亲戚", "其他的亲戚", "路德维卡的侄女", "路德维卡的妹夫", "一个远亲", "一个亲戚",
    "刚才那对夫妇", "舅父们", "那对夫妇", "家庭教师", "格林纳伯爵", "格林讷伯爵", "格拉夫·格林讷", "犯人母亲", "施瓦岑贝柯侯爵", "马科斯公爵",
    "久洛·安德拉希伯爵", "马科斯&索菲", "史蒂凡·卡罗伊伯爵", "埃勒梅尔·巴卡尼伯爵", "鲁契尼", "教授", "学生", "学生2", "艺术家", "记者", "流浪艺人", "流浪艺人2", "诗人", "鲁&诗人",
    "客人1", "客人2", "教授&学生", "流浪艺人&诗人", "学生&教授", "宫女", "宫女们", "女人们", "男人们", "众人", "所有人", "病人们", "病院相关管理人员", "女疯子", "女疯子&伊丽莎白",
    "伯爵夫人", "伯爵夫人&宫女", "伯爵", "侯爵", "男爵", "医生", "医生（死神）", "美发师", "红衣大主教劳施尔", "肯佩恩男爵", "劳施尔", "法官", "死亡", "随从",
    "伊丽莎白", "伊丽莎白姑母", "伊丽莎白的姑母", "路德维拉公爵夫人", "伊丽莎白&马科斯", "一粒沙&死神&弗兰茨", "索菲&宫女",
  ];
  const leaked = sandbox.window.songs.flatMap((song) => song.lines)
    .filter((line) => speakerLabels.some((label) => new RegExp(`^${label}[：:]`).test(line.zh || "")));
  assert.equal(leaked.length, 0);
  assert.equal(
    sandbox.window.songs.flatMap((song) => song.lines).some((line) => line.id === "elisabeth-das-musical-04-052"),
    false,
  );
  assert.doesNotMatch(wordDataJs, /"aaaaaaaaaaaaaaa":/u);
});
...
test("Starmania opening keeps the reviewed Chinese translation", () => {
  if ("elisabeth-das-musical" !== "starmania") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Moulin Rouge keeps source IPA corrections when lyric text is unchanged", () => {
  if ("elisabeth-das-musical" !== "moulin-rouge") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Phantom and Love Never Dies stay in separate source ranges", () => {
  if (!["phantom-of-the-opera", "love-never-dies"].includes("elisabeth-das-musical")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  const songs = sandbox.window.songs;
  if ("elisabeth-das-musical" === "phantom-of-the-opera") {
...
test("Romeo Aimer keeps one opening lyric, not an expanded spelling duplicate", () => {
  if ("elisabeth-das-musical" !== "romeo-et-juliette") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("Notre-Dame speaker metadata never enters the lyric IPA or speech input", () => {
  if ("elisabeth-das-musical" !== "notre-dame-de-paris") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
...
test("reviewed OCR word fragments are reassembled", () => {
  if (!["la-legende-du-roi-arthur", "phantom-of-the-opera", "notre-dame-de-paris", "le-roi-soleil", "mozart-opera-rock"].includes("elisabeth-das-musical")) return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
  const lines = sandbox.window.songs.flatMap((song) => song.lines);
  ...
  if ("elisabeth-das-musical" === "la-legende-du-roi-arthur") {
...
test("Le Roi Soleil keeps clean lyric breaks and real glosses", () => {
  if ("elisabeth-das-musical" !== "le-roi-soleil") return;
  const sandbox = { window: {} };
  vm.runInNewContext(songsJs, sandbox);
  vm.runInNewContext(wordDataJs, sandbox);
...
test("Moliere keeps the requested show name and dictionary meaning", () => {
  if ("elisabeth-das-musical" !== "moliere-le-spectacle-musical") return;
  const sandbox = { window: {} };
  vm.runInNewContext(wordDataJs, sandbox);
